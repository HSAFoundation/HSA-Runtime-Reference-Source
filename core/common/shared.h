...
class BaseShared {
 public:
  static void SetAllocateAndFree(std::function<void*(size_t, size_t)>& allocate,
...
class Shared : public BaseShared {
 public:
  Shared() {
    ...
    shared_object_ = reinterpret_cast<T*>(allocate_->operator()(sizeof(T), A));
    std::memset(shared_object_, 0, sizeof(T));
  }
